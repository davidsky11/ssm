...
/**
 * @ClassName	ConcurrentLRUHashMap.java
 * @Description	基于ConcurrentHashMap修改的LRUMap 
 * 		LRU即Least Recently Used（最近最少使用）缓存替换策略。
 * 		在任何LRU算法中，它必定有以下两个策略组成：
 * 			1、退化策略。根据访问情况，对节点按热度进行排序（hot->cold），以便决定哪些节点是热节点（hot）的，哪些节点是冷节点（cold）的。
 * 			这个退化策略，一般按以下两种方式去处理：
 * 				|- 非集中式。即每命中一次就进行退化操作。非集中式的退化操作，往往由双向链表的方式去实现。每次命中之后就移动命中节点在链表中的位置。（位置靠前的就是hot的数据）。当然，复杂的策略中，有用queue数组进行hot分级等。
 * 				|- 集中式。即定期去进行退化操作。在集中式的退化操作，常用的策略是：每次命中之后，记录一个时间戳、定时器时间点等等参数。由一个线程去扫描，定期清除老数据。
 * 			2、清除策略。即去掉那些cold的数据
 * 				|- 替换。这个早操作系统缓存中应该是一个常用的做法。
 * 				|- 删除。删除掉数据，以腾出空间放心的数据。（因为内存是有限的）。
 * 
 * 		分段实现锁分离 + 每个段内维护一份退化链表
 * 		实现策略：
 * 			1、锁分离机制。内部分成了多个segement，每个segement是独立加锁，相互不干扰。
 * 			2、每个segement内部维护一个双向链表（退化链表）。每次命中/添加，就把节点移动到退化链表头部。
 * 			3、每次put操作，通过hash，散列到每个segement中，判断segement的容量是否达到阈值。如果达到阈值，则删除退化链表中最末尾的节点。
 * @Author 		kevin
 * @CreateTime	2016年8月30日 上午11:08:31
 * @Version 	V1.0
 */
