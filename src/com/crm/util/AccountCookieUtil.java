 ...
 *
 */
public class AccountCookieUtil {
	 ...
	 * @param response
	 */
	public static void saveCookie(Account account, HttpServletResponse response) throws Exception {
		// cookie的有效期至（到哪一天）
		long validTime = System.currentTimeMillis() + (cookieMaxAge * 1000);
		// MD5加密用户详细信息（其实就是把当前用户加密一下，后面判断是否是同一个用户）
		String cookieValueWithMd5 = MD5.md5crypt(account.getUsername()+ ":"+ account.getPassword() + ":" + validTime + ":" + Const.WEBKEY);
		// 将要被保存的完整的Cookie值
		String cookieValue = account.getUsername() + ":" + validTime +":"+cookieValueWithMd5;
