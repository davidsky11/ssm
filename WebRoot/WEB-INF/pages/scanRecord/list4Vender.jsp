	...
	<div class="row">
		<div class="box">
			<div class="box-body">
						...
						<table class="table table-hover center">
							...
							<c:forEach items="${srs}" var="sr" varStatus="status">
								<tr>
									...
									<td>${status.count}</td>
									<td><c:if test="${sr.user != null}">
										${sr.user.username}
										<c:if test="${sr.user.userAlias != null }">
										(${sr.user.userAlias})
										</c:if>
										</c:if> <c:if test="${sr.user == null}">
										${sr.userName}
									</c:if></td>
									<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss"
											value="${sr.scanTime}" /></td>
									<td><c:if test="${sr.activity != null}">
										${sr.activity.title}
									</c:if></td>
									<c:if test="${userType eq '3'}">
										<td><c:if test="${sr.award != null}">
									...
									<td>
										<button id="detailBtn" type="button"
											...
											onclick="detailItem('${sr.id}');">详情</button>
								</tr>
							</c:forEach>
						</table>
					</div>
				...
				<!-- /.box-body -->
				<!-- 分页 -->
				<vino:pagination page="${page}" action="scanRecord/list4Vender"
